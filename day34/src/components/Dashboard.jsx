...
function Dashboard() {
  ...
  const handleDelete = async (taskId) => {
    await deleteDoc(doc(db,"Tasks",taskId)).then(()=>{
      let filterData = taskData.filter((e,i)=>e.docId != taskId);
  ...
  return (
    <>
      <section className="bg-black flex justify-between flex-wrap items-center px-12 py-3">
      ...
      <section className="bg-gray-700 py-5">
        <form onSubmit={handleSubmit} className="flex flex-col items-center lg:flex-row justify-center gap-10 lg:gap-20" action="">
          <input
            type="text"
            value={formTask.task}
            name="task"
